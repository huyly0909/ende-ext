...
build:
	...
	@cp $(VERSIONS_DIR)/$(ZIP_NAME) $(DIST_DIR)/ende.zip
	@echo ""
	@echo "✅ $(UNPACKED_DIR)/         → Load unpacked in Chrome"
	@echo "✅ $(DIST_DIR)/ende.zip   → 🚀 File to upload to GitHub Release"
	@echo "✅ $(VERSIONS_DIR)/$(ZIP_NAME) → Archived version"
	@echo "   Size: $$(du -h $(DIST_DIR)/ende.zip | cut -f1)"
